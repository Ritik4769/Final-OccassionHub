// VanueUpdatePasswordModal.js-// export default EditProfileModal;
...
function VanueUpdatePasswordModal(props) {
    ...
    const handleSubmitProfileUpdateData = async (event) => {
        ...
        try {
            updatedPassData.Id = props.data._id;
            const result = await axios.post(venue_requestUrl + "/updateVenuePassword", updatedPassData);
            if (result.status === 201) {
                alert("Password updated successfully");
                setFormSubmitted(true); // Update state to hide the form
            }
        } catch (err) {
            console.log("Error while updating Venue profile");
        }
    }
...
export default VanueUpdatePasswordModal;
