...
function CatererResponseModal(props) {
    const {userData,date} = props;
    ...
    const handleSubmitResponse = async (event) => {
        ...
        try {
            const response = await axios.post(caterre_requestUrl + "/sendResponseToUserPrice", {catereid:props.userData.catereid, diseasPrice,date:props.userData.date });
...
export default CatererResponseModal;